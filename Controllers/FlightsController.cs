...
namespace FlightBooking.Controllers
{
    public class FlightsController : Controller
    {
        ...
        public async Task<IActionResult> SearchFlight(){
            ...
            var schedules = from s in _context.Schedules.Include(s => s.DepartureAirport).Include(s => s.DestinationAirport).Include(s=>s.Airline).ThenInclude(a => a.AirlineCompany) select s;
        ...
        {
            ViewBag.Schedule = _context.Schedules.Include(s => s.Airline).ThenInclude(a => a.AirlineCompany).Where(s => s.Id == scheduleId).FirstOrDefault();
